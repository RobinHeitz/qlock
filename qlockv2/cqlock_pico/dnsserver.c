...
static int dns_socket_new_dgram(struct udp_pcb **udp, void *cb_data,
...
static void dns_server_process(void *arg, struct udp_pcb *upcb, struct pbuf *p,
  ...
  *answer_ptr++ = 4;                  // length
  ...
  dns_socket_sendto(&d->udp, &dns_msg, answer_ptr - dns_msg, src_addr,
                    src_port);
...
void dns_server_deinit(dns_server_t *d) { dns_socket_free(&d->udp); }
