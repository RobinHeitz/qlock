...
def minute_pixels(minutes:int) -> list[int]:
    """Returns a list of pixel indices (int) representing the current minutes.
    
    Params: 
    - minutes (int): Current minutes ranging from [0, 59].
    """
    MIN_POINTS_DEF = {1:WD_MIN_1,2:WD_MIN_2,3:WD_MIN_3,4:WD_MIN_4}
    return MIN_POINTS_DEF.get(minutes % 5, [])


def hour_pixels(hour:int):
    """Returns a list of pixel indices (int) representing the current hour."""
    return HOUR_DEF.get(hour, [])
